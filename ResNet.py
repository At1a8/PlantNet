...
def basic_bn_stem(model, data, **kwargs):
    ...
    #p = model.Conv(p, 'conv12', 64, dim, 3, pad=1, stride=2, no_bias=1)
    #p = model.AffineChannel(p, 'res_conv12_bn', dim=dim, inplace=True)
    #p = model.Relu(p, p)

    #p = model.Conv(p, 'conv13', 64, dim, 3, pad=1, stride=2, no_bias=1)
    #p = model.AffineChannel(p, 'res_conv13_bn', dim=dim, inplace=True)
    #p = model.Relu(p, p)
